...
# node_address=$(basechaind keys list | grep  "address: " | cut -c12-)
# current_date=$(date -u +"%Y-%m-%dT%TZ")
# cat $HOME/.basechaind/config/genesis.json | jq -r --arg current_date "$current_date" '.app_state["claims"]["params"]["airdrop_start_time"]=$current_date' > $HOME/.basechaind/config/tmp_genesis.json && mv $HOME/.basechaind/config/tmp_genesis.json $HOME/.basechaind/config/genesis.json

# # Set claims records for validator account
# amount_to_claim=10000
# cat $HOME/.basechaind/config/genesis.json | jq -r --arg node_address "$node_address" --arg amount_to_claim "$amount_to_claim" '.app_state["claims"]["claims_records"]=[{"initial_claimable_amount":$amount_to_claim, "actions_completed":[false, false, false, false],"address":$node_address}]' > $HOME/.basechaind/config/tmp_genesis.json && mv $HOME/.basechaind/config/tmp_genesis.json $HOME/.basechaind/config/genesis.json

# # Set claims decay
# cat $HOME/.basechaind/config/genesis.json | jq -r --arg current_date "$current_date" '.app_state["claims"]["params"]["duration_of_decay"]="1000000s"' > $HOME/.basechaind/config/tmp_genesis.json && mv $HOME/.basechaind/config/tmp_genesis.json $HOME/.basechaind/config/genesis.json
# cat $HOME/.basechaind/config/genesis.json | jq -r --arg current_date "$current_date" '.app_state["claims"]["params"]["duration_until_decay"]="100000s"' > $HOME/.basechaind/config/tmp_genesis.json && mv $HOME/.basechaind/config/tmp_genesis.json $HOME/.basechaind/config/genesis.json

# # Claim module account:
# # 0xA61808Fe40fEb8B3433778BBC2ecECCAA47c8c47 || basechain15cvq3ljql6utxseh0zau9m8ve2j8erz84vyscz
# cat $HOME/.basechaind/config/genesis.json | jq -r --arg amount_to_claim "$amount_to_claim" '.app_state["bank"]["balances"] += [{"address":"basechain15cvq3ljql6utxseh0zau9m8ve2j8erz84vyscz","coins":[{"denom":"abasecoin", "amount":$amount_to_claim}]}]' > $HOME/.basechaind/config/tmp_genesis.json && mv $HOME/.basechaind/config/tmp_genesis.json $HOME/.basechaind/config/genesis.json
...
if [[ $1 == "pending" ]]; then
  if [[ "$OSTYPE" == "darwin"* ]]; then
      sed -i '' 's/create_empty_blocks_interval = "0s"/create_empty_blocks_interval = "30s"/g' $HOME/.basechaind/config/config.toml
      sed -i '' 's/timeout_propose = "3s"/timeout_propose = "30s"/g' $HOME/.basechaind/config/config.toml
      sed -i '' 's/timeout_propose_delta = "500ms"/timeout_propose_delta = "5s"/g' $HOME/.basechaind/config/config.toml
      sed -i '' 's/timeout_prevote = "1s"/timeout_prevote = "10s"/g' $HOME/.basechaind/config/config.toml
      sed -i '' 's/timeout_prevote_delta = "500ms"/timeout_prevote_delta = "5s"/g' $HOME/.basechaind/config/config.toml
      sed -i '' 's/timeout_precommit = "1s"/timeout_precommit = "10s"/g' $HOME/.basechaind/config/config.toml
      sed -i '' 's/timeout_precommit_delta = "500ms"/timeout_precommit_delta = "5s"/g' $HOME/.basechaind/config/config.toml
      sed -i '' 's/timeout_commit = "5s"/timeout_commit = "150s"/g' $HOME/.basechaind/config/config.toml
      sed -i '' 's/timeout_broadcast_tx_commit = "10s"/timeout_broadcast_tx_commit = "150s"/g' $HOME/.basechaind/config/config.toml
  else
      sed -i 's/create_empty_blocks_interval = "0s"/create_empty_blocks_interval = "30s"/g' $HOME/.basechaind/config/config.toml
      sed -i 's/timeout_propose = "3s"/timeout_propose = "30s"/g' $HOME/.basechaind/config/config.toml
      sed -i 's/timeout_propose_delta = "500ms"/timeout_propose_delta = "5s"/g' $HOME/.basechaind/config/config.toml
