	...
	testCases := []struct {
	...
	}{
		{
			...
			banktypes.Metadata{
				...
				Name:        "basechain",
				Symbol:      "basechain",
				Description: "EVM, staking and governance denom of basechain",
				DenomUnits: []*banktypes.DenomUnit{
					{
						Denom:    "abasecoin",
						Exponent: 0,
						Aliases:  []string{"atto basechain"},
					},
					{
						Denom:    "basechain",
						Exponent: 18,
			...
			banktypes.Metadata{
				...
				DenomUnits: []*banktypes.DenomUnit{
					...
					},
					{
						Denom:    "ubasechain",
						Exponent: 12,
						Aliases:  []string{"micro basechain"},
					},
					{
						Denom:    "basechain",
						Exponent: 18,
...
func TestEqualAliases(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			"different lengths",
			[]string{},
			[]string{"atto basechain"},
			false,
		},
		{
			"different values",
			[]string{"attobasechain"},
			[]string{"atto basechain"},
			false,
		},
		{
			"same values, unsorted",
			[]string{"atto basechain", "abasecoin"},
			[]string{"abasecoin", "atto basechain"},
			false,
		},
		{
			"same values, sorted",
