...
var _ = Describe("Onboarding: Performing an IBC Transfer followed by autoswap and convert", Ordered, func() {
	...
	Describe("from a non-authorized channel: Cosmos ---(uatom)---> Basechain", func() {
		BeforeEach(func() {
		...
		It("No swap and convert operation - abasecoin balance should be 0", func() {
			nativebasechain := s.basechainChain.App.(*app.Basechain).BankKeeper.GetBalance(s.basechainChain.GetContext(), receiverAcc, "abasecoin")
			Expect(nativebasechain).To(Equal(coinbasechain))
		})
		It("Basechain chain's IBC voucher balance should be same with the transferred amount", func() {
			ibcAtom := s.basechainChain.App.(*app.Basechain).BankKeeper.GetBalance(s.basechainChain.GetContext(), receiverAcc, uatomIbcdenom)
	...
	Describe("from an authorized channel: Gravity ---(uUSDC)---> Basechain", func() {
		When("ERC20 contract is deployed and token pair is enabled", func() {
			BeforeEach(func() {
				...
				s.FundBasechainChain(sdk.NewCoins(ibcBalance))
			...
			Context("when no swap pool exists", func() {
				...
				It("Convert: Basechain chain's IBC voucher balance should be same with the original balance", func() {
					ibcUsdc := s.basechainChain.App.(*app.Basechain).BankKeeper.GetBalance(s.basechainChain.GetContext(), receiverAcc, uusdcIbcdenom)
				...
				It("Convert: ERC20 token balance should be same with the transferred IBC voucher amount", func() {
					erc20balance := s.basechainChain.App.(*app.Basechain).Erc20Keeper.BalanceOf(s.basechainChain.GetContext(), contracts.ERC20MinterBurnerDecimalsContract.ABI, tokenPair.GetERC20Contract(), common.BytesToAddress(receiverAcc.Bytes()))
			...
			Context("when swap pool exists", func() {
				...
				When("Basechain chain's abasecoin balance is 0", func() {
					BeforeEach(func() {
						result = s.SendAndReceiveMessage(s.pathGravitybasechain, s.IBCGravityChain, "uUSDC", 10000000000, sender, receiver, 1)
					})
					It("Swap: balance of abasecoin should be same with the auto swap threshold", func() {
						autoSwapThreshold := s.basechainChain.App.(*app.Basechain).OnboardingKeeper.GetParams(s.basechainChain.GetContext()).AutoSwapThreshold
						nativebasechain := s.basechainChain.App.(*app.Basechain).BankKeeper.GetBalance(s.basechainChain.GetContext(), receiverAcc, "abasecoin")
				...
				When("Basechain chain's abasecoin balance is between 0 and auto swap threshold (3basechain)", func() {
					BeforeEach(func() {
						s.FundBasechainChain(sdk.NewCoins(sdk.NewCoin("abasecoin", sdkmath.NewIntWithDecimal(3, 18))))
						result = s.SendAndReceiveMessage(s.pathGravitybasechain, s.IBCGravityChain, "uUSDC", 10000000000, sender, receiver, 1)
					})
				...
				When("Basechain chain's abasecoin balance is bigger than the auto swap threshold (4basechain)", func() {
					BeforeEach(func() {
						s.FundBasechainChain(sdk.NewCoins(sdk.NewCoin("abasecoin", sdkmath.NewIntWithDecimal(4, 18))))
						result = s.SendAndReceiveMessage(s.pathGravitybasechain, s.IBCGravityChain, "uUSDC", 10000000000, sender, receiver, 1)
					})
		...
		When("ERC20 contract is deployed and token pair is enabled", func() {
			BeforeEach(func() {
				...
				s.basechainChain.App.(*app.Basechain).Erc20Keeper.SetTokenPair(s.basechainChain.GetContext(), *tokenPair)
				sender = s.IBCGravityChain.SenderAccount.GetAddress().String()
				receiver = s.basechainChain.SenderAccount.GetAddress().String()
				senderAcc = sdk.MustAccAddressFromBech32(sender)
				receiverAcc = sdk.MustAccAddressFromBech32(receiver)
			...
			It("No convert: Basechain chain's IBC voucher balance should be same with (original balance + transferred amount - swapped amount)", func() {
				events := result.GetEvents()
				var sdkEvents []sdk.Event
				for _, event := range events {
					sdkEvents = append(sdkEvents, sdk.Event(event))
